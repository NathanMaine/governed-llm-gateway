...
## Work slices
1) **Scaffold**: `src/`, config loader, types, basic logging helper.
2) **Routing + provider adapter**: resolve alias → provider; call provider client stub.
3) **Rate limiting**: per-client in-memory limiter; friendly error shape.
4) **Telemetry/logging**: log request metadata, outcome, usage (approx if provider returns).
5) **Examples**: sample config, sample curl, README run notes.
...
- Starts with sample config and runs locally.
- `/v1/chat` accepts a request with `client_id`, `model`, `messages` and forwards to the configured provider.
- Per-client in-memory rate limit enforced; returns a structured rate-limit error.
- Logs each request with model alias, provider, client id, outcome, and usage when available.
